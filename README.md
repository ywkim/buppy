...
Buppy와의 상호작용을 위해 다음과 같은 슬래시 명령어를 사용할 수 있습니다:

- `/config`: 현재 Buppy의 설정을 확인할 수 있습니다. 챗 모델, 시스템 프롬프트, 온도 등의 설정을 보여줍니다. 다음과 같이 사용하세요:

  ```
  /config
  ```

  이 명령을 사용하면 Buppy가 현재 설정된 값을 슬랙 채널에 메시지로 보내줍니다.
